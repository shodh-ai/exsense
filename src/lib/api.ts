...
export class ApiService {
  ...
  async reorderLessons(courseId: string, orderedLessonIds: string[]): Promise<void> {
    return this.client.patch(`/api/courses/${courseId}/lessons/reorder`, {
      orderedLessonIds
    } as any);
  }
  ...
  async getAdminAnalyticsOverview(): Promise<{
    users: number;
    courses: number;
    enrollments: number;
    lessons: number;
  ...
  async createOrGetWhiteboardSession(payload: {
    enrollmentId?: string | null;
    courseId?: string | null;
    roomName?: string | null;
    title?: string | null;
...
// Use this when you need to create an API service outside of a React component
