...
import CourseMap from "@/components/CourseMap"; // --- MODIFIED: Component imported ---
...
const implementationItems = [
  {
    ...
    description: "Weekly sessions to review participant input and adjust strategies.",
  },
  {
    title: "Mentorship Program Expansion",
    description: "Establish connections to foster peer learning and support.",
  },
];
        ...
        <ul className="flex flex-col items-start gap-4 w-full animate-fade-in [--animation-delay:1200ms]">
          {achievements.map((achievement, index) => (
            <li key={`achievement-${index}`} className="flex items-start gap-3 w-full">
              <CheckIcon className="w-5 h-5 text-[#40bb33] flex-shrink-0 mt-0.5" />
        ...
        <div className="flex flex-col items-start gap-4 w-full animate-fade-in [--animation-delay:1800ms]">
          {improvementPoints.map((point, index) => (
            <div key={index} className="flex items-start gap-3 w-full">
              <CheckCircleIcon className="w-5 h-5 text-[#394169] flex-shrink-0 mt-0.5" />
              <p className="flex-1 font-['Plus_Jakarta_Sans',_sans-serif] font-medium text-base text-[#394169]">
                {point}
              </p>
            </div>
          ...
          {implementationItems.map((item, index) => (
            <div key={index} className="flex flex-col items-start gap-2 w-full">
              <div className="flex items-start gap-3 w-full">
                <CheckCircleIcon className="w-5 h-5 text-[#566fe9] flex-shrink-0 mt-0.5" />
                <div className="flex-1 font-['Plus_Jakarta_Sans',_sans-serif] font-semibold text-base text-[#394169]">
                  {item.title}
                </div>
              </div>
              <div className="pl-8 w-full">
                <div className="font-['Plus_Jakarta_Sans',_sans-serif] font-medium text-base text-[#394169]">
                  {item.description}
                </div>
              </div>
            </div>
...
const ActionButtonsSection = ({ studentName }: { studentName: string }): JSX.Element => (
    <section className="w-full animate-fade-in [--animation-delay:3000ms] flex flex-col gap-4">
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full">
        <Button
          variant="outline"
          className="w-full h-auto px-6 py-4 rounded-[100px] border-2 border-[#566fe9] text-[#566fe9] font-semibold text-base hover:bg-[#566fe9] hover:text-white transition-colors"
        >
          Update Course Map
        </Button>
        <Button
        ...
        <main className="flex-grow overflow-y-auto">
          <div className="relative z-10 mx-auto w-full max-w-[1440px] px-4 py-8 sm:px-6 md:py-12">
            ...
            <div className="mx-auto flex w-full max-w-[850px] flex-col items-start gap-12 md:gap-16">
              ...
              <ActionPlanSection />
              {/* --- MODIFIED: Old section replaced with the new component --- */}
              <CourseMap />
