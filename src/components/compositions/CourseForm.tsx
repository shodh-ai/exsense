...
import { useCourse, useCreateCourse, useDeleteCourse, useUpdateCourse } from "@/hooks/useApi";
  ...
  const handleSave = async () => {
    ...
    if (isEditMode) {
      try {
        ...
        await updateCourseMutation.mutateAsync({ courseId: cid, courseData: formState });
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (statusCode === 401) {
        ...
        toast.error(`Error: Could not delete the course. ${errorMessage}`);
      }
    } finally {
      setIsDeleting(false);
  ...
  return (
    <>
        ...
        <div className="max-w-4xl mx-auto px-6 py-8 pb-20">
          ...
          <div className="h-full space-y-4 mt-6">
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              >
                <span className="text-sm font-semibold">{isDeleting ? 'Deleting...' : (isEditMode ? 'Delete Course' : 'Cancel')}</span>
              </Button>
      ...
      <ConfirmationModal
        ...
        title="Delete Course"
        message="Are you sure you want to delete this course? This action cannot be undone and will permanently remove all course content, lessons, and student progress."
